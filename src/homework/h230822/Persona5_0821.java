...
class Arsene //공통부분으로 상위클래스를 만들어서 오버라이드 할 수 있을 것 같습니다.
{
	...
	static final String summonSpeech = "내 이름은 황혼의 약탈자 아르센! 다시금 나의 힘이 필요한가...? 좋다. 마음껏 휘둘러 보거라.";
	
	//생성할 때 호출
	String[] specificity = {"배수의 닻","배턴 터치를 받았을 때, 포위당했을 때의 전용 스킬을 사용할 수 있다","계승 가능"};
	//효과를 불러올때는 0번으로 계승여부확인은 2번으로
...
class Pixie{
	...
	String[] specificity = {"추가 전도체","다운된 적에 대한 감전율이 상승한다","계승 가능"};
	//효과를 불러올때는 0번으로 계승여부확인은 2번으로
...
class MixPersona //합성
{
	...
	void mix()
	{
		...
		for(int i = 0; i <fusion.length; i++)//조건이 뭘까? 수동? 규칙?
			{
				if(fusion[i].equals(resultArcana))
				{
					mixPersonaResult=resultArcana;
					break;
				}
				else if(fusion[i].equals(resultArcanaReverse))
				{
					mixPersonaResult=resultArcana;
					break;
				}
		...
		resultLevel=a.level/p.level+1;
		
		
		
		//소지페르소나 = 전체페르소나(mixPersonaResult아르카나,resultLevel레벨) 모양으로 일치하는걸 찾아야한다
		}
	}
